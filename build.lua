...
local function build_dynamic()
    ...
    build_static_library()
    utils.run(CC, utils.concat(CFLAGS, { "-s", "-o", LUA_EXENAME, "lua.c", "lua54.dll" }, LINKFLAGS))
    utils.run(CC, utils.concat(CFLAGS, { "-s", "-o", LUAC_EXENAME, "luac.c", LUA_LIBNAME }, LINKFLAGS))
...
if BUILD_AS_DLL then
    ...
    utils.run("gendef", { "lua54.dll" })
    utils.run("dlltool", { "-d", "lua54.def", "-m", "i386:x86-64", "-l", LUA_LIBNAME })
...
utils.run("install", { "-d", INSTALL_BIN .. "/lua" })

utils.run("install", { "src/" .. LUA_EXENAME, INSTALL_BIN })
utils.run("install", { "src/" .. LUAC_EXENAME, INSTALL_BIN })
if BUILD_AS_DLL then utils.run("install", { "src/" .. "lua54.dll", INSTALL_BIN }) end
